...
async function getInteraction(req, res) {
  ...
  const cookieValue = ssHandler.get(
    ctx.cookies,
  ...
  /**
   * Fetch client ID from either:
   * 1. x-logto-app-id header: for Experience API requests sent from Logto Experience UI
   * 2. app_id query parameter: general case for OIDC requests and URL redirections
   */
  const clientId = ctx.headers['logto-app-id'] || new URL(ctx.request.url, this.issuer).searchParams.get('app_id');

  /**
   * Fetch interaction ID from the interaction cookie map by the given client ID
   */
  ...
  // Note: Empty client ID is exempted for backward compatibility
