...
export class Provider extends Koa {
  ...
  #int = {
    dynamicClients: new LRU({ maxSize: 100 }),
  };
  ...
  constructor(issuer, setup) {
    ...
    if (configuration.features.openid4vci.enabled) {
      try {
        this.#int.CNonceChallenges = new ServerChallenge(
          configuration.features.openid4vci.nonceSecret,
          'OpenID4VCI c_nonce',
        );
      } catch (cause) {
        throw new TypeError('features.openid4vci.nonceSecret secret must be a 32-byte Buffer instance', { cause });
      }
    }
  ...
  get PreAuthorizedCode() {
    this.#PreAuthorizedCode ||= models.getPreAuthorizedCode(this);
