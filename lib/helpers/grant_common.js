...
export async function issueIdToken(ctx, source, at, grant, {
  ...
  const token = new IdToken({
    ...await getCtxAccountClaims(
      ...
      'id_token',
      /**
       * LOGTO PATCH(id-token-claims-effective-scope): query the account claims with the
       * effective scope of the current request (`scopeOverride || source.scopes`) instead of
       * the source token's full stored scope. For upstream's claims mechanism the two are
       * interchangeable — the IdToken mask below filters by the effective scope either way —
       * but claim sources that load data per scope (as Logto's does) would otherwise
       * over-fetch on down-scoped refresh requests, and fail them on errors from queries the
       * request no longer needs. Only the refresh_token grant passes `scopeOverride`; for the
       * other grants `[...scopes].join(' ')` reconstructs `source.scope` exactly.
       *
       * Upstream: typeof source.scope === 'string' ? source.scope : [...scopes].join(' '),
       */
      [...scopes].join(' '),
      claims,
      rejected,
