...
export async function findGrantSource(ctx, Model, value, label) {
...
export async function consumeGrantSource(ctx, source, label) {
  if (source.consumed) {
...
export async function applyDpopBinding(ctx, dPoP, at) {
  if (dPoP) {
    await checkDpopReplay(ctx, dPoP, ctx.oidc.client.clientId, InvalidGrant);
