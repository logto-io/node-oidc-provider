...
export default function initializeApp() {
  ...
  async function ensureSessionSave(ctx, next) {
    try {
      ...
      /**
       * LOGTO PATCH(ensure-session-save-persisted-only): only persist a session that was
       * persisted before. `persist()` saves without extending the lifetime, so it requires an
       * existing `exp` and throws otherwise; this guard mirrors that precondition. The device
       * user-code routes reach here with a brand new session when their error handler writes an
       * xsrf secret onto it, and skipping is safe: those sessions never got a session cookie
       * written either, so the secret could never be matched back on any later request.
       */
      if (
        ctx.oidc.session?.touched
        && !ctx.oidc.session.destroyed
        && typeof ctx.oidc.session.exp === 'number'
      ) {
  ...
  for (const { handler, parameters, grantType } of Object.values(grants)) {
    ...
    if (configuration.grantTypes.has(grantType) && !grantTypeHandlers.has(grantType)) {
      ...
      this.registerGrantType(grantType, handler, grantParameters, dupes);
    }
  }
  ...
  if (features.openid4vci.enabled) {
    ...
    get('openid_credential_issuer', credentialIssuerRoute, CORS.open, error(this, 'openid_credential_issuer.error'), openidCredentialIssuer);
    options('cors.openid_credential_issuer', credentialIssuerRoute, CORS.open, CORS.respond);

    post('credential', routes.credential, error(this, 'credential.error'), CORS.credential, ...credential);
