...
async function getAttestationSignaturePublicKey(ctx, header, payload, client) {
  // @param ctx - koa request context
  // @param header - Protected Header of the Client Attestation JWT
  // @param payload - decoded, untrusted payload of the Client Attestation JWT
...
async function assertAttestationJwtAndPop(ctx, attestation, pop, client) {
  ...
  // @param client - client making the request
  //
  // When features.attestClientAuth.additionalSecuritySignal is enabled this function
  // is also invoked after regular client authentication succeeds. At that point the
  // attestation is available as an additional request security signal.
}
...
async function openid4vciGetKeyAttestationSignaturePublicKey(ctx, iss, header, client) {
  ...
  mustChange('features.openid4vci.getKeyAttestationSignaturePublicKey', 'be able to verify the Key Attestation JWT signature');
...
async function openid4vciIssueCredential(
  ...
  mustChange('features.openid4vci.issueCredential', 'issue and return one or more credentials for a validated credential request');
...
function PreAuthorizedCodeTTL(ctx, code, client) {
  shouldChange('ttl.PreAuthorizedCode', 'define the expiration for PreAuthorizedCode artifacts');
  return 10 * 60; // 10 minutes in seconds
}
...
function makeDefaults() {
  const defaults = {
    ...
    features: {
       ...
       * features.dPoP
       *
       * title: [RFC9449](https://www.rfc-editor.org/info/rfc9449/) - OAuth 2.0 Demonstration of Proof-of-Possession at the Application Layer (DPoP)
       *
      ...
      dPoP: {
         ...
         *   Nonces are derived from this secret rather than stored; the same value MUST be
       ...
       * features.mTLS
       *
       * title: [RFC8705](https://www.rfc-editor.org/info/rfc8705/) - OAuth 2.0 Mutual TLS Client Authentication and Certificate Bound Access Tokens (MTLS)
       *
       ...
       *
       * title: [draft-ietf-oauth-attestation-based-client-auth-10](https://www.ietf.org/archive/id/draft-ietf-oauth-attestation-based-client-auth-10.html) - OAuth 2.0 Attestation-Based Client Authentication
       *
       ...
       *   JWT generated by the Client Instance. It can also enable Client Attestation as
       *   an additional security signal alongside existing Client Authentication methods
       *   using the `attestation_pop_jwt` Proof-of-Possession method.
       *
       */
      attestClientAuth: {
        ack: undefined,
        ...
        /*
         * features.attestClientAuth.additionalSecuritySignal
         *
         * description: Specifies whether Client Attestation shall be accepted or required
         *   as an additional security signal alongside regular client authentication. Use
         *   `optional` to validate the signal when it is present, or `required` to require
         *   the OAuth-Client-Attestation and OAuth-Client-Attestation-PoP headers after
         *   the client is identified. This uses the `attestation_pop_jwt` method and does
         *   not enable DPoP combined mode.
         */
         ...
         *   Challenges are derived from this secret rather than stored; the same value MUST be
         *   configured on all instances of a deployment and kept stable across restarts.
         * @nodefault
         */
         ...
         *
         * description: Specifies a helper function that shall be invoked to retrieve the public
         *   key used for Client Attestation JWT signature verification. At the point of this
         *   function's invocation, only the JWT format has been validated; no cryptographic or
         *   claims verification has occurred.
         *
         ...
         * }));
         *
         * function getAttestationSignaturePublicKey(ctx, header, payload, client) {
         *   if (typeof header.jku === 'string' && attesters.has(header.jku)) return attesters.get(header.jku)(header);
         *   throw new Error('unsupported oauth-client-attestation attester');
         * }
         * ```
         */
       ...
       *
       * title: [draft-ietf-oauth-client-id-metadata-document-02](https://www.ietf.org/archive/id/draft-ietf-oauth-client-id-metadata-document-02.html) - OAuth Client ID Metadata Document (CIMD)
       *
       * description: Specifies whether the authorization server shall support resolving client
       *   metadata from Client Identifier URLs used as `client_id` values. When enabled, if a
       *   `client_id` is an HTTPS URL conforming to the specification's requirements, the
       *   authorization server shall fetch the Client ID Metadata Document from that URL and use
       *   it as the client's
      ...
      clientIdMetadataDocument: {
         ...
         *   ID Metadata Document from a Client Identifier URL. This function enables enforcement of
         ...
         *   resolved from a Client ID Metadata Document is about to be used, including when served from
        ...
        /**
         * features.clientIdMetadataDocument.transformClientMetadata
         *
         * description: `LOGTO PATCH(cimd-metadata-transform)`, not an upstream helper.
         *   Specifies a helper function that shall be invoked on every resolution of a Client
         *   ID Metadata Document — including when the document is served from cache — after
         *   the document passes the specification checks and before the Client instance is
         *   constructed. It receives a copy of the raw document and must return the metadata
         *   object to construct the Client from; the returned value goes through the regular
         *   client metadata schema validation, and the cached raw document is never affected.
         */
         ...
         *   applied to HTTP cache headers when caching fetched Client ID Metadata Documents.
       ...
       *
       * title: [RFC6749](https://www.rfc-editor.org/info/rfc6749/#section-1.3.4) - Client Credentials
       *
       ...
       * features.deviceFlow
       *
       * title: [RFC8628](https://www.rfc-editor.org/info/rfc8628/) - OAuth 2.0 Device Authorization Grant (Device Flow)
       *
       ...
       *
       * title: [RFC7662](https://www.rfc-editor.org/info/rfc7662/) - OAuth 2.0 Token Introspection
       *
       ...
       *
       * title: [RFC9701](https://www.rfc-editor.org/info/rfc9701/) - JWT Response for OAuth Token Introspection
       *
       ...
       *
       * title: [RFC9126](https://www.rfc-editor.org/info/rfc9126/) - OAuth 2.0 Pushed Authorization Requests (PAR)
       *
       ...
       *
       * title: [OIDC Dynamic Client Registration 1.0](https://openid.net/specs/openid-connect-registration-1_0-errata2.html) and [RFC7591](https://www.rfc-editor.org/info/rfc7591/) - OAuth 2.0 Dynamic Client Registration Protocol
       *
       ...
       *
       * title: [RFC7592](https://www.rfc-editor.org/info/rfc7592/) - OAuth 2.0 Dynamic Client Registration Management Protocol
       *
       ...
       *
       * title: [RFC9396](https://www.rfc-editor.org/info/rfc9396/) - OAuth 2.0 Rich Authorization Requests
       *
       ...
       * @important
       *
       * title: [RFC8707](https://www.rfc-editor.org/info/rfc8707/) - Resource Indicators for OAuth 2.0
       *
       ...
       *
       * title: [OIDC Core 1.0](https://openid.net/specs/openid-connect-core-1_0-errata2.html#RequestObject) and [RFC9101](https://www.rfc-editor.org/info/rfc9101/#name-passing-a-request-object-by) - Passing a Request Object by Value (JAR)
       *
       ...
       * features.revocation
       *
       * title: [RFC7009](https://www.rfc-editor.org/info/rfc7009/) - OAuth 2.0 Token Revocation
       *
       ...
       * features.openid4vci
       *
       * title: [OpenID for Verifiable Credential Issuance 1.0](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0-final.html)
       *
       * description: Specifies whether OpenID4VCI core capabilities shall be enabled. When enabled,
       *   the authorization server shall expose the Credential Issuer Metadata, Credential
       *   Endpoint, and Nonce Endpoint routes, and perform protocol validation for issuance
       *   requests. Supported proof types are `jwt` and `attestation`.
       *   The `attestation` proof type relies on Key Attestation JWTs
       *   signed by a Wallet Provider; use `getKeyAttestationSignaturePublicKey`
       *   to resolve the attester's public key.
       *
       *   Credential Offer is an application-level concern outside the scope of the
       *   framework. The Issuer constructs the Credential Offer JSON object (containing
       *   `credential_issuer`, `credential_configuration_ids`, and `grants`) and delivers it to
       *   the Wallet via a custom URL scheme redirect (same-device) or QR code (cross-device).
       *   The `issuer_state` authorization parameter, included in the offer's
       *   `grants.authorization_code` object and sent back by the Wallet in the authorization
       *   request, should be registered via `extraParams` with a validator callback. Once
       *   registered, it becomes available in `ctx.oidc.params` and is included in the
       *   interaction session details automatically. The Wallet's `credential_offer_endpoint`
       *   client metadata can be supported via `extraClientMetadata` if needed.
       *   The `metadata` configuration property below can be used to add any additional
       *   Credential Issuer Metadata members.
       *
       *   Access to the Credential Endpoint requires an Access Token issued through a
       *   user-facing authorization grant (e.g. Authorization Code). The token MUST use the
       *   `opaque` format and its audience MUST equal the value returned by the
       *   `credentialEndpointExpectedAudience` helper. Deployments shall use the
       *   `features.resourceIndicators` mechanism to configure that same value as a resource
       *   indicator. Use the `defaultResource` helper to detect credential-requesting
       *   authorization requests and return it as the resource so that the client needs not to
       *   use the `resource` parameter. Use the `useGrantedResource` helper to return `true` so
       *   that the issued Access Token targets the Credential Endpoint rather than the UserInfo
       *   Endpoint.
       *
       * example: Configuring resourceIndicators for the Credential Endpoint.
       *
       * ```js
       * {
       *   features: {
      ...
      openid4vci: {
        ...
        /**
         * features.openid4vci.nonceSecret
         *
         * description: Specifies the cryptographic secret used to generate and validate
         *   OpenID4VCI `c_nonce` challenges exposed by the nonce endpoint.
         *   This value MUST be a 32-byte Buffer instance.
         *   `c_nonce` values are derived from this secret rather than stored; the same value MUST be
         *   configured on all instances of a deployment and kept stable across restarts.
         * @nodefault
         */
        nonceSecret: undefined,
        ...
        /*
         * features.openid4vci.preAuthorizedCodeGrant
         *
         * description: Specifies whether the OpenID4VCI Pre-Authorized Code Flow shall be
         *   enabled. When enabled, the authorization server shall accept
         *   `urn:ietf:params:oauth:grant-type:pre-authorized_code` grant type exchanges at
         *   the token endpoint. Clients using this grant type must have it registered in
         *   their `grant_types` client metadata.
         *
         *   Pre-authorized codes represent issuance authorization obtained through means
         *   outside of the protocol exchanges defined by this framework. Creating them,
         *   together with their underlying Grant, is an application-level concern, as is
         *   delivering them to the Wallet inside a Credential Offer.
         *
         *   Pre-authorized codes are single-use. An optional Transaction Code (`txCode`
         *   property, a string) may be attached to a pre-authorized code, in which case the
         *   Wallet-provided `tx_code` parameter presence is validated before the code is
         *   consumed and its value is then compared in constant time, a failed comparison
         *   revokes the pre-authorized code and its underlying Grant.
         *
         *   No ID Token is issued as part of this grant type's token exchange.
         *
         * example: Minting a pre-authorized code.
         *
         * ```js
        ...
        /*
         * features.openid4vci.metadata
         *
         * description: Free-form object with additional top-level members to be merged
         *   into the Credential Issuer Metadata response.
         */
        metadata: {},

        /*
         * features.openid4vci.credentialConfigurationsSupported
         *
         * description: Specifies static Credential Issuer metadata values for
         *   `credential_configurations_supported`.
         *
         * example: Defining credential configurations.
         *
         * ```js
         * {
        ...
        /*
         * features.openid4vci.credentialEndpointExpectedAudience
         *
         * description: Specifies a helper function that shall be invoked to resolve the value
         *   the Access Token's `aud` claim must equal in order to access the Credential
         *   Endpoint. It shall return a non-empty string.
         *
         *   The default derives the Credential Endpoint URL from the incoming request, which
         *   only resolves consistently when the Credential Endpoint and the Token Endpoint are
         *   served on the same host. Deployments serving the Credential Endpoint on another
         *   host, such as a mutual-TLS host, shall return a fixed absolute URL from this helper.
         *
         *   Whatever this helper returns MUST equal the resource indicator the Access Token was
         *   issued for; this helper and the `features.resourceIndicators` configuration are two
         *   halves of the same contract. Note that OpenID4VCI recommends the Credential Issuer
         *   Identifier (`ctx.oidc.issuer`) as the `resource` parameter value, which is another
         *   value that does not vary with the host serving the request.
         */
        credentialEndpointExpectedAudience: openid4vciCredentialEndpointExpectedAudience,

        /*
         * features.openid4vci.credentialConfigurationPolicy
         *
         * description: Specifies a helper function that shall be invoked at runtime to
         *   decide whether a specific credential configuration is currently issuable for
         *   the current request context.
         *
         * example: Allowing issuance only for a specific client and granted scope.
         *
         * ```js
         * {
         ...
         * ```
         */
        credentialConfigurationPolicy: openid4vciCredentialConfigurationPolicy,

        /*
         * features.openid4vci.issueCredential
         *
         * description: Specifies a helper function that shall be invoked to perform actual
         *   credential issuance and return credential response payloads. By the time this
         *   function is called all proof signatures, algorithms, types, required claims
         ...
         *
         *   - `jwt` The value is the original array of compact JWS strings. When the JWT
         *     proof(s) contain a `key_attestation` JOSE header parameter, the pre-parsed
         *     key attestation data is available as `proofs.key_attestation` with:
         *     - `jwt` {string} The Key Attestation JWT compact serialization.
         *     - `attestedKeys` {Object[]} The `attested_keys` claim (array of JWK objects).
         *     - `payload` {Object} The full Key Attestation JWT payload, including optional
         *       claims such as `key_storage`, `user_authentication`, and `certification`.
         *
         ...
         *     - `payload` {Object} The full Key Attestation JWT payload, including optional
         *       claims such as `key_storage`, `user_authentication`, and `certification`
         *       when present. If `key_attestations_required` is configured for the credential
         *       configuration, the required claims have been validated to contain at least one
         *       matching value before this function is called.
         *
         * example: Returning one credential in the response.
         *
         * ```js
         * {
         ...
         *
         * description: Specifies a helper function used to resolve the public key for verifying
         *   Key Attestation JWT (typ `key-attestation+jwt`) signatures when the `attestation`
         *   proof type is used. At the point of invocation the JWT format and `iss` claim
         *   presence have been validated; no cryptographic or further claims verification has
         *   occurred yet.
         *
         *   The function receives the Koa request context, the `iss` (Wallet Provider identifier)
         *   from the Key Attestation JWT, the decoded JOSE protected header, and the Client
         *   instance. It MUST return a public key as a CryptoKey, KeyObject, or JWK object.
         *   If the function throws, the credential endpoint responds with `invalid_proof`.
         *
         * example: Fetching attester public keys from the attester's hosted JWKS
         *
         * ```js
         ...
         * ```
         */
        getKeyAttestationSignaturePublicKey: openid4vciGetKeyAttestationSignaturePublicKey,
      },
    },
     ...
     *   [JWK Set format](https://www.rfc-editor.org/info/rfc7517/#section-5) as defined in
     ...
     * @important
     *
     * title: [RFC7636](https://www.rfc-editor.org/info/rfc7636/) - Proof Key for Code Exchange (PKCE)
     *
    ...
    ttl: {
      ...
      PreAuthorizedCode: PreAuthorizedCodeTTL,
