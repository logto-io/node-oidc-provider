...
function getCache(provider) {
  ...
  if (!entries) {
    entries = new LRU({ maxSize: 100 });
    cache.set(provider, entries);
  }
  return entries;
}

function getPending(provider) {
  let requests = pending.get(provider);
  if (!requests) {
    requests = new Map();
    pending.set(provider, requests);
  }
  return requests;
}
 ...
 * The hook receives a copy and only its return value is used, so a host transform can
 * never reach the cached raw document — the cache must keep the document untouched for
 * the transform to see it unmodified on every resolution.
 */
async function applyMetadataTransform(ctx, feature, properties) {
  const metadata = await feature.transformClientMetadata(ctx, structuredClone(properties));
  if (typeof metadata !== 'object' || metadata === null || Array.isArray(metadata)) {
    throw new TypeError('transformClientMetadata must return the client metadata object');
  }
  return metadata;
}
...
async function fetchMetadataDocument(provider, id, cacheDuration) {
  let response;
  try {
    ...
    throw new InvalidClient('client_id metadata document fetch failed', { cause });
  }

  if (response.status !== 200) {
    throw new InvalidClient('client_id metadata document fetch failed', `unexpected response status ${response.status}`, { cause: response });
  }
  ...
  return {
    properties,
    maxAge: parseCacheDuration(response, cacheDuration) * 1000,
  };
}

async function createClient(provider, feature, ctx, properties) {
  // LOGTO PATCH(cimd-metadata-transform)
  const client = await addClient(provider, await applyMetadataTransform(ctx, feature, properties), { cimd: true });
  Object.defineProperty(client, 'clientIdMetadataDocument', { value: true });
  return client;
}

async function assertClientAllowed(feature, ctx, client) {
  if (!(await feature.allowClient(ctx, client))) {
    throw new InvalidClient('client is not allowed');
  }
}

async function resolveCachedClient(provider, feature, ctx, properties) {
  const client = await createClient(provider, feature, ctx, properties);
  await assertClientAllowed(feature, ctx, client);
  return client;
}
...
export async function resolveClientByMetadataDocument(provider, id) {
  ...
  let properties = entries.get(id);
  if (properties) {
    return resolveCachedClient(provider, feature, ctx, properties);
  }
  ...
  // Another request may have populated the cache while allowFetch was pending.
  properties = entries.get(id);
  if (properties) {
    return resolveCachedClient(provider, feature, ctx, properties);
  }

  const requests = getPending(provider);
  let request = requests.get(id);
  if (!request) {
    request = fetchMetadataDocument(provider, id, feature.cacheDuration);
  ...
  try {
    ...
    client = await createClient(provider, feature, ctx, fetched.properties);

    if (!entries.has(id)) {
      entries.set(id, fetched.properties, { maxAge: fetched.maxAge });
    }
  } finally {
