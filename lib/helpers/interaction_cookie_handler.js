...
export function setInteractionUid(cookieValue, clientId, uid) {
   ...
   * just-written client and the _legacy key are never evicted, so a single oversized entry
   * is kept rather than dropped.
   */
  const evictable = Object.keys(mapping)
    .filter((key) => key !== '_legacy' && key !== String(clientId));

  let encoded = encodeURIComponent(JSON.stringify(mapping));
  while (
    evictable.length
    && (evictable.length + 1 > MAX_CLIENT_ENTRIES || encoded.length > MAX_ENCODED_LENGTH)
  ) {
