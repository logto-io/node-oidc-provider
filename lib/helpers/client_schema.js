...
export default function getSchema(provider) {
  ...
  class Schema {
    ...
    #initialize(metadata) {
      ...
      const responseTypes = new Set(this.response_types.flatMap((rt) => rt.split(' ')));
    ...
    webUris() {
      WEB_URI.forEach((prop) => {
        if (this[prop] !== undefined) {
          const isAry = ARYS.includes(prop);
          (isAry ? this[prop] : [this[prop]]).forEach((val) => {
            const method = HTTPS_URI.includes(prop) ? validUrl.isHttpsUri : validUrl.isWebUri;
            const type = method === validUrl.isWebUri ? 'web' : 'https';
            if (!method(val)) {
              this.invalidate(isAry
    ...
    redirectUris(uris = this.redirect_uris, label = 'redirect_uris') {
      uris.forEach((redirectUri) => {
        ...
        /**
         * LOGTO PATCH(redirect-uri-relaxation): Logto's Console and Management API accept
         * any http, https, or custom-scheme URI for every application type and surface
         * best-practice warnings in the UI instead, so the application_type-specific shape
         * restrictions (web clients: https/http only; native clients: http loopback-only,
         * claimed-HTTPS non-loopback-only) are lifted. Upstream's forbidden-scheme check,
         * which only guarded native clients, now applies to both application types.
         */
        if (FORBIDDEN_SCHEMES.has(protocol)) {
          this.invalidate(`${label} must not use the ${protocol.slice(0, -1)} URI scheme`);
        }

        if (this.application_type === 'web' && this.grant_types.includes('implicit')) {
          if (protocol === 'http:') {
