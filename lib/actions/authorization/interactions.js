...
export default async function interactions(resumeRouteName, ctx, next) {
  ...
  // Get current interaction cookie value
  const cookieName = oidc.provider.cookieName('interaction');
  const currentCookieValue = ssHandler.get(
    ctx.oidc.cookies,
    cookieName,
    cookieOptions,
  );

  // Update the cookie with client-specific UID mapping
  const clientId = ctx.oidc.client.clientId;
  const newCookieValue = setInteractionUid(currentCookieValue, clientId, uid);
